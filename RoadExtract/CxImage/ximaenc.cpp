...
bool CxImage::Load(const TCHAR* filename, double top, double left, double right,double bottom, DWORD imagetype)
{
	if(Load(filename,imagetype))
	{
		int w,h,tmp;
		w = GetWidth();
		h = GetHeight();
		...
		rc.left = fabs(left)*w;
		rc.top = fabs(top)*h;
		rc.right = fabs(right)*w;
		rc.bottom = fabs(bottom)*h;
		...
		if(Crop(rc))
		{
			//Save(_T("c:\\a\\1.png"),CXIMAGE_FORMAT_PNG);
			return true;
		}
	}
	return false;
